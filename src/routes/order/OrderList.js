...
function OrdertList({ dispatch, history, order, router }) {
    ...
    },
    {
      ...
      render(record) {
        return (record.product_snapshot || []).map((i) => {
          return (
            <span key={`${record.order_no}_${i.id}`}>
              {i.name}　x{i.num}
              <br />
            </span>
          )
      ...
      render(record) {
        return (
          <div className={cs.tableAction}>
            <span
              onClick={() => {
                history.push(`/${router.model}/detail/${record.order_no}`)
              }}
            >
              查看详情
            </span>
  ...
  return (
    <div>
        ...
        <Row>
          <Col span={12}>
            ...
            <SearchInput
              ...
              bindName="product_name"
            />
            <SearchInput
              lable="货架编号"
              value={search.shelf_id}
              bindName="shelf_id"
            />
      ...
      <Mcard >
        <DataTable columns={columns} model={order} rowKey="order_no" />
      </Mcard>
    </div>
  )
}
